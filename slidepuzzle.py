...
def main(x, y, score):
    ...
    if PLAYING:
        ...
        SOLVEDBOARD = getStartingBoard(True) # a solved board is the same as the board in a start state.
        ...
        while PLAYING: # main game loop
            ...
            checkForQuit() # tuka stay go menjanjeto na playing 
...
def help(board):
    ...
    message = 'Possible moves: ' + ', '.join(moves)

    textSurf, textRect = makeText(message, MESSAGECOLOR, BGCOLOR, 5, 30)
    DISPLAYSURF.blit(textSurf, textRect)
    highlightTiles(board, blankx, blanky, RED)
    #treba sosedite na blank da svetat crveno

#Change 2
def highlightTiles(board, blankx, blanky, color):
    if blanky != 0:
        drawTile(blankx, blanky - 1, board[blankx][blanky - 1],color)
    if blanky != BOARDHEIGHT - 1:
        drawTile(blankx, blanky + 1, board[blankx][blanky + 1],color)
    if blankx != 0:
        drawTile(blankx - 1, blanky, board[blankx - 1][blanky],color)
    if blankx != BOARDWIDTH - 1:
        drawTile(blankx + 1, blanky, board[blankx + 1][blanky],color)
...
def getStartingBoard(solved):
    ...
    if not solved:
        blankxpos, blankypos = BOARDWIDTH-1, BOARDHEIGHT-1
    print(str(blankxpos)+ " " + str(blankypos))
...
def makeMove(board, move):
    ...
    if move == UP:
        ...
        blankxpos, blankypos = blankx, blanky+1
    elif move == DOWN:
        board[blankx][blanky], board[blankx][blanky - 1] = board[blankx][blanky - 1], board[blankx][blanky]
        blankxpos, blankypos = blankx, blanky-1
    elif move == LEFT:
        board[blankx][blanky], board[blankx + 1][blanky] = board[blankx + 1][blanky], board[blankx][blanky]
        blankxpos, blankypos = blankx+1, blanky
    elif move == RIGHT:
        board[blankx][blanky], board[blankx - 1][blanky] = board[blankx - 1][blanky], board[blankx][blanky]
        blankxpos, blankypos = blankx-1, blanky
    
    # print("MOVE"+str(blankxpos) + " " + str(blankypos))
...
#Change 2
def drawTile(tilex, tiley, number,color, adjx=0, adjy=0):
...
def slideAnimation(board, direction, message, animationSpeed):
    ...
    for i in range(0, TILESIZE, animationSpeed):
        ...
        if direction == UP:
            drawTile(movex, movey, board[movex][movey], TILECOLOR,0, -i)
        if direction == DOWN:
            drawTile(movex, movey, board[movex][movey],TILECOLOR, 0, i)
        if direction == LEFT:
            drawTile(movex, movey, board[movex][movey], TILECOLOR,-i, 0)
        if direction == RIGHT:
            drawTile(movex, movey, board[movex][movey],TILECOLOR, i, 0)
